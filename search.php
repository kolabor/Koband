...
<div class="row author-rows"> 
        <?php
        $args_search_tour = array
        (       
             'post_type' => 'tour',   
             'post_staus'=> 'publish',
        );
        $search_tour_posts = new WP_Query( $args_search_tour );
        if ( $search_tour_posts->have_posts() ) : ?>
        <!--start loop-->
            <div class="divTable">
                <div class="divTableBody koband_post_tour">
                    ...
                    while ( $search_tour_posts->have_posts() ) : $search_tour_posts->the_post(); 
                        $post_id = get_the_ID();  
    ...
    <div class="row author-rows">
        ...
        $args_search_album = array(
            'post_type' => 'album',
            'post_staus'=> 'publish',


            );
        $args_search_singles = array(
            'post_type' => 'singles',
            'post_staus'=> 'publish',    
  
            ); 
        $search_album_posts = new WP_Query($args_search_album); ?>
        <?php if ($search_album_posts->have_posts() ) : ?>
            <?php while( $search_album_posts->have_posts() ) : $search_album_posts->the_post();
        ...
        <div class="container">
            <div class="row album-head border_first_color main_font_color">
                <div class="col-sm-1"><?php the_post_thumbnail(array(70,70));?></div>
                    ...
                    <div class="col-sm-3 main_font_color"><?php _e('Date:<br>', 'koband');?> <span class="main_font_color"><?php if(isset($album_date[0]))  { echo  $album_date[0]; } ?></span></div>
                    <div class="col-sm-3 main_font_color"><?php _e('Length:<br>', 'koband');?><span class="main_font_color"><?php if(isset($album_length[0])) { echo  $album_length[0]; } ?></span></div>
                ...
                <div class="container album-songs-show-hide">
                    ...
                    <div class="row album-head border_first_color">
                        <div class="col-sm-4 songs-head main_font_color"><?php _e('Song Name', 'koband');?></div>
                        <div class="col-sm-4 songs-head main_font_color"><?php _e('Song Length', 'koband');?></div>
    ...
    <div class="row author-rows">
        <?php $search_single_posts = new WP_Query($args_search_singles); ?>
        <?php if ($search_single_posts->have_posts() ) : ?>
            <?php while ($search_single_posts->have_posts() ) : $search_single_posts->the_post();
                $post_id = get_the_ID(); ?>
    ...
    <div class="row author-rows">
        <?php
        $args_search_media = array(    
             'post_type' => 'media',   
             'post_staus'=> 'publish',
             
        );
        $search_media_posts = new WP_Query($args_search_media);
        if ($search_media_posts->have_posts() ) : ?>
        <!-- start loop -->
            <?php 
            while ( $search_media_posts->have_posts() ) : $search_media_posts->the_post();
            $post_id = get_the_ID(); ?>
